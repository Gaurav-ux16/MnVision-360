...
export const MapInspectionDrawer: React.FC<MapInspectionDrawerProps> = ({
  ...
  return (
    <div className="bg-[#070D1E] border border-slate-800 rounded-xl shadow-2xl p-5 text-white font-sans space-y-5 relative max-h-[820px] overflow-y-auto select-none">
      {/* Drawer Close Button */}
      <button
        onClick={onClose}
      ...
      <div className="space-y-1 border-b border-slate-800 pb-3 pr-8">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[11px] font-mono font-bold text-[#C5A059] uppercase tracking-wider">
        ...
        <p className="text-xs text-slate-400 font-mono flex items-center gap-1.5">
          ...
          <span className="text-amber-300 font-bold">{latitude.toFixed(4)}° N, {longitude.toFixed(4)}° E</span>
        </p>
      </div>

      {/* Primary Prospectivity Score & Classification */}
      <div className="bg-[#0A1128] p-4 rounded-xl border border-slate-800 space-y-3">
        <div className="flex items-center justify-between">
          ...
          <span className={`px-2 py-0.5 rounded text-[10px] font-mono font-bold uppercase ${
            ...
            isModerateProspectivity ? 'bg-amber-950 text-amber-300 border border-amber-800' :
        ...
        <div className="flex items-baseline justify-between pt-1">
          <div>
            ...
            <div className="text-[11px] text-slate-400 font-mono mt-0.5">
              Prospectivity Percentile: <strong className="text-amber-300">{prospectivity.percentile.toFixed(1)}th %ile</strong>
            </div>
          </div>
        ...
        <div className="w-full h-2 bg-slate-900 rounded-full overflow-hidden border border-slate-800">
          <div
            className={`h-full rounded-full transition-all duration-500 ${
              isHighProspectivity ? 'bg-gradient-to-r from-amber-500 to-red-600' :
              isModerateProspectivity ? 'bg-gradient-to-r from-emerald-500 to-amber-400' :
      ...
      <div className="grid grid-cols-2 gap-3 font-mono text-xs">
        ...
        <div className="p-3 bg-slate-950/80 rounded-lg border border-slate-800 space-y-1">
          ...
          <span className="text-[9px] text-slate-500 block">OOD Score: <strong className="text-amber-300">{confidence.ood_applicability_score}</strong></span>
        </div>
      </div>
      ...
      {nearest_target && (
        <div className="p-3 bg-amber-950/30 border border-amber-800/60 rounded-lg flex items-center justify-between text-xs font-mono">
          <div className="space-y-0.5">
            <span className="text-[10px] text-amber-300 font-bold uppercase block">NEAREST TARGET ASSOCIATED</span>
          ...
          {onOpenTargetDetail && (
            <button
              ...
              className="px-2.5 py-1 rounded bg-[#C5A059] text-slate-950 font-bold text-[10px] hover:bg-amber-400 transition flex items-center gap-1"
            >
      ...
      <div className="pt-2 border-t border-slate-800 text-[10px] text-slate-400 font-mono space-y-1">
        <div className="flex items-center justify-between text-slate-500">
          ...
          <span className="text-amber-400 font-bold">{dataset_provenance}</span>
        </div>
